...
def normalize_conventional_commit_message(message: str) -> str:
	...
	if len(candidate) >= 5 and any(c.isalpha() for c in candidate) and not candidate.startswith("chore: "):
		subject = candidate
	...
	if extracted_scope and not has_scope_line(body_lines):
		body_lines = [f"Scope: {extracted_scope}", "", *body_lines] if body_lines else [f"Scope: {extracted_scope}"]
